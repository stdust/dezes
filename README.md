...
- 🎨 **Flexible Theming & Visual Customization**
  - Rich built-in main themes: `dark`, `light`, `gray`, `latte`, `arctic_ice`, `coffee`, `darkone`, `ice`, `matrix`, `mocha`, `paper`, `punk`, `terracotta`.
  - Dedicated disassembly themes: `disasm`, `dark`, `light`, `grey` (`:set disasmtheme <name>`).

- ⚙️ **Automatic Configuration (`.dzsrc`)**
  - Automatically creates `.dzsrc` in the executable directory on initial run if not found, initialized with the `# dezes initialization configuration` header.

- 🌐 **Multi-language Support (i18n)**
  - Full interface support for English (`en`), Korean (`ko`), and Chinese (`zh`) (`:set lang <en|ko|zh>`).
