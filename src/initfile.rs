...
impl App {
    ...
    pub fn read_initfile(&mut self) -> Result<(), Box<dyn Error>> {
        ...
        // 1. Directory where the executable is located (exe_dir)
...
pub fn merge_initfile(existing: Option<&str>, enc1: &str, enc2: &str, lang: &str, theme: &str, disasmtheme: &str) -> String {
    ...
    const OWNED: [(&str, &[&str]); 5] = [
        ...
        ("lang", &["lang", "language"]), ("theme", &["theme"]), ("disasmtheme", &["disasmtheme", "disasm_theme"]),
    ];

    let value_for = |slot: &str| match slot {
        "enc1" => enc1,
        "enc2" => enc2,
        "theme" => theme, "disasmtheme" => disasmtheme, _ => lang,
    };
...
set byteline 16
";
        let merged = merge_initfile(Some(existing), "EUC-KR", "UTF-16LE", "ko", "dark", "dark");

        assert!(merged.contains("set bitness 32"));
        assert!(merged.contains("set hintbar off"));
    ...
    fn the_order_of_the_file_is_kept() {
        ...
        let merged = merge_initfile(Some(existing), "EUC-KR", "none", "en", "dark", "dark");
        let lines: Vec<&str> = merged.lines().collect();

        assert_eq!(lines[0], "set theme dark");
    ...
    fn the_users_spelling_survives() {
        ...
        let merged = merge_initfile(Some(existing), "EUC-KR", "UTF-8", "ko", "dark", "dark");
    ...
    fn line_endings_are_preserved() {
        ...
        let merged = merge_initfile(Some(crlf), "EUC-KR", "none", "en", "dark", "dark");
        assert!(merged.contains("\r\n"));
        assert!(!merged.replace("\r\n", "").contains('\n'), "mixed endings: {:?}", merged);

        let lf = "set theme grey3\nset enc1 UTF-8\n";
        let merged = merge_initfile(Some(lf), "EUC-KR", "none", "en", "dark", "dark");
    ...
    fn a_duplicate_setting_is_collapsed() {
        ...
        assert!(merged.contains("set enc1 EUC-KR"));
        assert!(merged.contains("set theme dark"));
    }
    ...
    fn merging_is_idempotent() {
        let existing = "# mine\nset theme grey3\nset enc1 UTF-8\nset enc2 none\nset lang en\n";
        let once = merge_initfile(Some(existing), "EUC-KR", "UTF-8", "ko", "dark", "dark");
        let twice = merge_initfile(Some(&once), "EUC-KR", "UTF-8", "ko", "dark", "dark");
    ...
    fn other_commands_are_not_mistaken_for_settings() {
        ...
        let merged = merge_initfile(Some(existing), "UTF-8", "none", "en", "dark", "dark");
