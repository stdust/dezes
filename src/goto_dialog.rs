        ...
        match key.code {
            ...
            KeyCode::Enter => {
                ...
                if let Some(addr) = crate::commands::eval_address_expression(app, raw_input) {
                    ...
                    // Interpreted the same way the box was pre-filled: as a VA in VA
                    // mode, as a file offset otherwise. This used to translate through
                    // `va_to_offset` unconditionally, so in offset mode `94D+1000`
                    // jumped to 0xD4D instead of 0x194D.
                    ...
                    if target_offset < filesize {
                        ...
                        crate::beep!();
                        App::log(app, format!("Address 0x{:X} out of bounds", addr));
                        app.state = UIState::Normal;
                        app.dialog_renderer = None;
                    }
                } else {
                    crate::beep!();
                    App::log(app, format!("Invalid address expression: '{}'", raw_input));
                    app.state = UIState::Normal;
                    app.dialog_renderer = None;
                }
            }
