...
mod header_edit_bounds_tests {
    ...
    fn in_bounds_edit_is_still_applied() {
        ...
        assert_eq!(app.hex_view.changed_bytes.get(&0x40).map(String::as_str), Some("44"));
        assert_eq!(app.hex_view.changed_bytes.get(&0x41).map(String::as_str), Some("33"));
        assert_eq!(app.hex_view.changed_bytes.get(&0x42).map(String::as_str), Some("22"));
        assert_eq!(app.hex_view.changed_bytes.get(&0x43).map(String::as_str), Some("11"));
    }
