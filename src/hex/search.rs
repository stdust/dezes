...
fn push_hex_pairs(s: &str, pattern: &mut Vec<HexPatternByte>, match_case: bool) -> Option<()> {
    ...
    for chunk in s.as_bytes().chunks(2) {
        ...
        if pair == "??" {
            ...
            if match_case {
                pattern.push(HexPatternByte::Exact(byte));
            } else {
...
pub fn find_all_pattern_matches(app: &mut App, pattern: &[HexPatternByte]) -> Vec<usize> {
    ...
    app.with_effective_buffer(|buffer| {
        ...
        let anchor = pattern.iter().enumerate().find_map(|(i, p)| match p {
            HexPatternByte::Exact(b) => Some((i, *b, *b)),
            HexPatternByte::IgnoreCase(b) => {
        ...
        match anchor {
            Some((anchor_idx, b1, b2)) => {
                ...
                while offset < haystack.len() {
                    let rel_pos = if b1 == b2 {
                        memchr::memchr(b1, &haystack[offset..])
                    } else {
                        memchr::memchr2(b1, b2, &haystack[offset..])
                    };
                    let Some(pos) = rel_pos else { break };
...
mod match_cap_tests {
    ...
    fn wildcard_only_pattern_is_capped() {
        ...
        let pattern = hex_string_to_pattern("??", false).expect("pattern");
    ...
    fn anchored_pattern_is_capped() {
        ...
        let pattern = hex_string_to_pattern("AA", false).expect("pattern");
    ...
    #[test]
    fn case_insensitive_pattern_matching_finds_all_cases() {
        let bytes = b"Hello hello HELLO HeLLo";
        let mut app = app_with(bytes);
        let pattern = vec![
            HexPatternByte::IgnoreCase(b'h'),
            HexPatternByte::IgnoreCase(b'e'),
            HexPatternByte::IgnoreCase(b'l'),
            HexPatternByte::IgnoreCase(b'l'),
            HexPatternByte::IgnoreCase(b'o'),
        ];
        let matches = find_all_pattern_matches(&mut app, &pattern);
        assert_eq!(matches, vec![0, 6, 12, 18]);
    }
}
