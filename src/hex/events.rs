...
pub(crate) fn parse_hex_or_text_bytes(
    ...
    // Try standard hex parsing first
    ...
    // Dump format parsing (lines starting with address e.g. 00000000 4D 5A 90 ...)
    let mut dump_bytes = Vec::new();
    let mut is_dump_format = false;
    for line in clean.lines() {
        ...
        // Dump row starts with address e.g. 00000000 or 00000000:
        if parts.len() >= 2 {
            let addr_part = parts[0].trim_end_matches(':');
            if (addr_part.len() == 8 || addr_part.len() == 16) && u64::from_str_radix(addr_part, 16).is_ok() {
                is_dump_format = true;
                // Parse hex byte tokens after address column
                for &token in &parts[1..] {
                    if token.len() == 2 {
                        if let Ok(b) = u8::from_str_radix(token, 16) {
                            dump_bytes.push(b);
                        } else {
                            break; // Stop at ASCII preview section
                        }
                    } else {
                        break; // Stop at non-2-digit hex
                    }
                }
            }
        }
...
pub fn hex_mode_events(app: &mut App, key: KeyEvent) -> Result<bool> {
    ...
    match key.code {
        ...
        KeyCode::Char('e') | KeyCode::Char('E') if key.modifiers.contains(KeyModifiers::CONTROL) => {
            hex::edit_dialog::open_edit_dialog(app);
        }
