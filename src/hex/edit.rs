...
pub fn displayed_byte(app: &App, offset: usize) -> u8 {
    if let Some(hex) = app.hex_view.changed_bytes.get(&offset)
        && let Ok(b) = u8::from_str_radix(hex.trim(), 16)
    {
        return b;
    }
    app.file_info
...
mod nibble_tests {
    ...
    fn a_single_digit_never_commits_a_half_byte() {
        ...
        let stored = app.hex_view.changed_bytes.get(&0x10).cloned();
        assert_eq!(
            stored.as_deref(),
            Some("7B"),
