    ...
    if let Event::Key(key) = event {
        ...
        match key.code {
            ...
            KeyCode::Enter => {
                ...
                // Kept for Alt+Left, which reopens this list.
                app.last_result = Some(UIState::DialogXref);
                app.dialog_renderer = None;
                app.state = UIState::Normal;
                return Ok(false);
            }
...
mod xref_copy_tests {
    ...
    fn y_and_shift_y_report_what_they_copied() {
        ...
        let _ = dialog_xref_events(&mut app, &Event::Key(key_c));
        let _ = dialog_xref_events(&mut app, &Event::Key(key_shift_c));
