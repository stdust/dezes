    ...
    match event {
        Event::Key(key) if key.kind == KeyEventKind::Press => {
            ...
            match app.state {
                ...
                UIState::HexSelection => {
                    // Global shortcuts (Ctrl+G, Ctrl+H, ':', F-keys, ...) work
                    // even while a selection is live, so a selection never
                    // silently eats a shortcut the user expects to work.
                    let state_before = app.state;
                    global::events::handle_global_events(app, key)?;
                    if app.state == state_before {
                        hex::selection::select_events(app, key)?;
                    }
                },
        ...
        Event::Mouse(_)
            if matches!(app.state, UIState::DialogStringRef) =>
        {
            crate::disasm::string_ref_dialog::dialog_string_ref_events(app, &event)?;
            return Ok(false);
        }
            ...
            if app.editor_view == crate::editor::AppView::Disasm {
                ...
                match mouse.kind {
                        ...
                        if in_rows =>
                    {
                        ...
                        let is_double_click = if let Some((last_time, last_row, _)) = app.last_left_click {
                            now.duration_since(last_time).as_millis() < 400 && last_row == mouse.row
                        } else {
                            false
                        };
                        ...
                        if is_double_click {
                            ...
                            crate::disasm::events::open_assemble_dialog(app);
                        } else {
                    ...
                    ratatui::crossterm::event::MouseEventKind::ScrollUp => {
                        let buffer = app.file_info.get_buffer_ref();
                        let mut curr_ofs = page_start;
                        for _ in 0..3 {
                            let curr_va = app.get_va(curr_ofs);
                            let advance = first_instr_len(buffer, curr_ofs, curr_va, bitness, filesize);
                            curr_ofs = curr_ofs.saturating_sub(advance);
                        }
                    ...
                    ratatui::crossterm::event::MouseEventKind::ScrollDown => {
                        ...
                        for _ in 0..3 {
                            let curr_va = app.get_va(curr_ofs);
                            let advance = first_instr_len(buffer, curr_ofs, curr_va, bitness, filesize);
                            curr_ofs = (curr_ofs + advance).min(filesize.saturating_sub(1));
                        }
                        app.reader.page_start = curr_ofs;
                        app.hex_view.offset = curr_ofs;
                        app.disasm_selection_anchor = None;
...
mod replace_dialog_tests {
    ...
    fn find_next_sees_pending_edits() {
        ...
        app.hex_view.changed_bytes.insert(0x10, 0xAA);
        app.hex_view.changed_bytes.insert(0x11, 0xBB);
    ...
    fn replace_one_preserves_earlier_edits_under_a_wildcard() {
        ...
        app.hex_view.changed_bytes.insert(0x10, 0x11);
        app.hex_view.changed_bytes.insert(0x11, 0xBB);
        app.hex_view.offset = 0x10;
        ...
        let at_10 = app.hex_view.changed_bytes.get(&0x10).copied();
        let at_11 = app.hex_view.changed_bytes.get(&0x11).copied();
        let status = app.hex_view.replace_dialog.status_message.clone();
        let _ = std::fs::remove_dir_all(&dir);

        assert_eq!(at_10, Some(0x22), "status was {:?}", status);
        assert_eq!(
            at_11,
            Some(0xBB),
    ...
    fn replace_all_sees_pending_edits() {
        ...
        app.hex_view.changed_bytes.insert(0x20, 0x77);

        set_patterns(&mut app, "77", "88");
        super::execute_replace_all(&mut app);

        let at_20 = app.hex_view.changed_bytes.get(&0x20).copied();
        let status = app.hex_view.replace_dialog.status_message.clone();
        let _ = std::fs::remove_dir_all(&dir);
    ...
    fn replace_works_without_any_edits() {
        ...
        let at_8 = app.hex_view.changed_bytes.get(&0x08).copied();
        let _ = std::fs::remove_dir_all(&dir);
        assert_eq!(at_8, Some(0x5A));
    }
}
