...
impl App {
    ...
    fn reparse_pe_headers(&mut self) -> bool {
        ...
        for (&ofs, hex_str) in &self.hex_view.changed_bytes {
            if ofs < prefix.len()
                && let Ok(b) = u8::from_str_radix(hex_str.trim(), 16)
            {
                prefix[ofs] = b;
            }
        }
    ...
    fn flush_changed_bytes(
        file: File,
        changed_bytes: &std::collections::HashMap<usize, String>,
    ) -> io::Result<usize> {
        let mut edits: Vec<(usize, u8)> = changed_bytes
            .iter()
            .filter_map(|(k, v)| u8::from_str_radix(v, 16).ok().map(|b| (*k, b)))
            .collect();
    ...
    pub fn write_block_to_file(&mut self, target_path: &Path) -> io::Result<usize> {
        ...
        for ofs in start..=actual_end {
            if let Some(s) = self.hex_view.changed_bytes.get(&ofs)
    ...
    pub fn read_u8(&mut self, offset: usize) -> Option<u8> {
        ...
        if let Some(hex_str) = self.hex_view.changed_bytes.get(&offset)
    ...
    pub fn save_initfile(&self) {
        ...
        // Startup directory, so `:set enc2` doesn't drop a config file into
        // whatever folder the file dialog was last browsing.
        let dir = crate::util::startup_dir();
...
mod block_dump_tests {
    ...
    fn block_is_written_exactly() {
        ...
        app.hex_view.changed_bytes.insert(0x12, "FF".to_string());
