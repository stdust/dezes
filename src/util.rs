...
fn query_ime_language_mode() -> &'static str {
    ...
    unsafe {
        // Our own console window first. `GetForegroundWindow` is whatever the user
        // is looking at, which may belong to another process - and the message below
        // is synchronous, so querying a busy foreign window blocked the render
        // thread. dezes also repaints while it is *not* focused, so that was not a
        // rare case.
